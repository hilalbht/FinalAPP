...
<style>
    ...
    .model-ok { color: #2e7d32; font-size: 0.85rem; font-weight: 500; padding: 0.5rem;
                background: #f0faf0; border-radius: 6px; border-left: 3px solid #4caf50; }
</style>
...
# ── Tahmin Fonksiyonu ─────────────────────────────────────────────────────────
def predict_with_model(inp):
    """Gerçek model ile tahmin."""
    row = pd.DataFrame([{
        "Drug":         inp["Drug"],
        "Age":          inp["Age"] * 365.25,   # yıl → gün (model bunu bekliyor)
        "Sex":          inp["Sex"],
        "Ascites":      inp["Ascites"],
        "Hepatomegaly": inp["Hepatomegaly"],
        "Spiders":      inp["Spiders"],
        "Edema":        inp["Edema"],
        "Bilirubin":    inp["Bilirubin"],
        "Cholesterol":  inp["Cholesterol"],
        "Albumin":      inp["Albumin"],
        "Copper":       inp["Copper"],
        "Alk_Phos":     inp["Alk_Phos"],
        "SGOT":         inp["SGOT"],
        "Tryglicerides":inp["Tryglicerides"],
        "Platelets":    inp["Platelets"],
        "Prothrombin":  inp["Prothrombin"],
    }])[features]

    probs = model.predict_proba(row)[0]
    # Model 3 sınıf mı 4 sınıf mı kontrol et
    n_classes = len(probs)
    if n_classes == 3:
        probs_4 = list(probs) + [0.0]
    else:
        probs_4 = list(probs)
    return probs_4
...
MODEL TAHMİNİ (XGBoost+RF+LightGBM Stacking, %89.9 doğruluk):
...
Cevabın Türkçe, klinik ve net olsun. Tedavi önerisi verme — sadece veri yorumu ve model açıklaması yap. Her madde için ayrı bir paragraf yaz."""
    ...
    models_list = ["gemini-2.0-flash-lite", "gemini-2.0-flash", "gemini-1.5-flash-8b"]
...
with st.sidebar:
    ...
    st.markdown("---")
    if MODEL_LOADED:
        st.markdown('<div class="model-ok">✅ Gerçek Model: Yüklendi (Stacking)</div>', unsafe_allow_html=True)
    else:
        st.error("❌ model.pkl yüklenemedi")
    ...
    if SECRETS_KEY:
        ...
        st.markdown("### 🔑 Gemini API Key (opsiyonel)")
        st.markdown("[Ücretsiz al →](https://aistudio.google.com/app/apikey)")
...
if not predict_btn:
    ...
    if not MODEL_LOADED:
        st.error(f"⚠️ model.pkl dosyası bulunamadı. Lütfen GitHub reposuna yükleyin.")
    st.markdown("""
    #### Bu sistem neler yapar?
    - **Histolojik evre tahmini** (Evre 1–4) — Gerçek eğitilmiş stacking modeli (XGBoost + RF + LightGBM)
    ...
    - **AI klinik analizi** — Gemini AI ile detaylı parametre yorumu (opsiyonel)
    """)
else:
    if not MODEL_LOADED:
        st.error("❌ model.pkl yüklenemedi. GitHub reposuna model.pkl dosyasını yükleyin.")
        st.stop()
